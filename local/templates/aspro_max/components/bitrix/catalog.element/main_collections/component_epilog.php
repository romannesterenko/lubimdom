...
$arTabOrder=[];
$arTabOrder[0] = 'nabors';
$arTabOrder[1] = 'series_elems';
$arTabOrder[2] = 'additional_products';
//dd($arTabOrder);
...
<?if($arResult["ID"]):?>
	...
	<?foreach($arBlockOrder as $code):?>
		...
		<?elseif($code == 'tabs'):?>
        <?//dd($templateData);
            $showNabors = count($templateData['PROPERTIES']['NABORS']['VALUE'])>0;
            $showSerieElems = count($templateData['PROPERTIES']['SERIE_ELEMS']['VALUE'])>0;
            $showAdditionalProducts = count($templateData['PROPERTIES']['ADDITIONAL_PRODUCTS']['VALUE'])>0;
            ?>
            <?if($showNabors || $showSerieElems || $showAdditionalProducts):?>
				<div class="ordered-block js-store-scroll">
						...
						<div class="tabs arrow_scroll">
							<ul class="nav nav-tabs font_upper_md">
								...
								<?foreach($arTabOrder as $value):?>
									<?if($value == "nabors"):?>
											<li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>"><a href="#nabors" data-toggle="tab">Готовые наборы</a></li>
									<?endif;?>
									<?if($value == "series_elems"):?>
											<li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>"><a href="#series_elems" data-toggle="tab">Элементы серии</a></li>
									<?endif;?>
									<?if($value == "additional_products"):?>
											<li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>"><a href="#additional_products" data-toggle="tab">Дополнительные товары</a></li>
									<?endif;?>
								<?endforeach;?>
							</ul>
						</div>
					...
					<div class="tab-content<?=($i <= 1 ? ' not_tabs' : '')?>">
						...
						<?foreach($arTabOrder as $value):?>
							<?if($value == "nabors"):?>
                                <div class="tab-pane <?=(!($iTab++) ? 'active' : '')?>" id="nabors">
                                    <?$APPLICATION->ShowViewContent('PRODUCT_NABORS_INFO')?>
                                </div>
							<?endif;?>
							<?if($value == "series_elems"):?>
                                <div class="tab-pane <?=(!($iTab++) ? 'active' : '')?>" id="series_elems">
                                    <?$APPLICATION->ShowViewContent('PRODUCT_SERIE_ELEMS_INFO')?>
                                </div>
                            <?endif;?>
                            <?if($value == "additional_products"):?>
                                <div class="tab-pane <?=(!($iTab++) ? 'active' : '')?>" id="additional_products">
                                    <?$APPLICATION->ShowViewContent('PRODUCT_ADDITIONAL_PRODUCTS_INFO')?>
                                </div>
                            <?endif;?>
						<?endforeach;?>
					</div>
...
<?$des->finishDynamicArea();?>
            ...
            <?$APPLICATION->ShowViewContent('MAYBE_YOU_LIKE')?>

            <?$APPLICATION->ShowViewContent('WITH_THIS_PRODUCT_BUY')?>
