...
if (!empty($arResult['ITEMS'])){
	...
	{
		//dd($arItem);
		$arResult['ITEMS'][$key]['COLLECTION_PRICE'] = 0;
		if($arItem['PROPERTIES']['ELEMENT_SHOW_PRICE']['VALUE']>0){
				$arPrice = CCatalogProduct::GetOptimalPrice($arItem['PROPERTIES']['ELEMENT_SHOW_PRICE']['VALUE']);
				if($arPrice['DISCOUNT_PRICE']>0){
						$arItem['COLLECTION_PRICE'] = number_format($arPrice['DISCOUNT_PRICE'], 0, ',', ' ');
				}else{
						$arItem['COLLECTION_PRICE'] = number_format($arPrice['RESULT_PRICE']['BASE_PRICE'], 0, ',', ' ');
				}
		}
		...
		$arItem['GALLERY'] = CMax::getSliderForItemExt($arItem, "ADDITIONAL_IMAGES", 'Y' == $arParams['ADD_DETAIL_TO_SLIDER']);
...
// print_r($arResult['ITEMS'][0]['MIN_PRICE']);
?>
